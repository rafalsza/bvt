...
profile_summary_file = "/user_data/" + "profile_summary.json"
# path to config file
# config_file = user_data_path + "/user_data/" + "config.yml"
config_file = "/user_data/" + "config.yml"
...
with kpi21:
    ...
    market_link = f'<value style="color: {market_perf_color}; text-decoration: none;" target="_blank" href="https://www.binance.com/en/trade/BTCUSDT">' + str(
        profile_summary.all_time_market_profit) + '</value>'
    st.markdown(
...
with kpi11:
    ...
    st.markdown(
        f"<h4 style='text-align: left; margin-left: 30px;'> Bot Performance: <span style='text-align: center; color: {bot_perf_color};'>{profile_summary.bot_profit_perc}%</span> <span> Est: </span><span style='color: {bot_perf_color}';>${profile_summary.bot_profit} {profile_summary.pair_with}</span></h3>",
        unsafe_allow_html=True)
...
try:
    ...
    open_columns = ["id", "buy_time", "symbol", "volume", "bought_at", "now_at", "change_perc", "profit_dollars",
                    "time_held", "tp_perc", "sl_perc", "buy_signal"]
    open_trades = transactions_df.loc[transactions_df['closed'] == 0, open_columns]

    closed_trades_columns = ["id", "buy_time", "symbol", "volume", "bought_at", "sold_at", "change_perc",
                             "profit_dollars", "sell_time", "time_held", "tp_perc", "sl_perc", "buy_signal",
                             "sell_reason"]
    closed_trades = transactions_df.loc[transactions_df['closed'] == 1, closed_trades_columns]

    st.markdown(
        f"### **Open Trades (Winning: <span style='color:green;'>{open_trades[open_trades.change_perc > 0].change_perc.count()}</span> | Losing: <span style='color:red;'>{open_trades[open_trades.change_perc <= 0].change_perc.count()}</span>) **",
        unsafe_allow_html=True)
